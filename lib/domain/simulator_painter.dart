...
class SimuladorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final paintEjes = Paint()..color = Colors.white30..strokeWidth = 1.5;
    final paintRegla = Paint()..color = Colors.white60..strokeWidth = 2.0;
    ...
    if (esModo2D) {
      ...
      canvas.drawLine(Offset(0, centroY), Offset(size.width, centroY), paintEjes);
      canvas.drawLine(Offset(centroX, 0), Offset(centroX, size.height), paintEjes);
      ...
      for (int i = -10; i <= 10; i++) {
        ...
        canvas.drawLine(Offset(posX, centroY - 5), Offset(posX, centroY + 5), paintRegla);
        
        final textSpanX = TextSpan(
          text: i.toString(),
          style: const TextStyle(color: Colors.white70, fontSize: 10),
        );
        final textPainterX = TextPainter(text: textSpanX, textDirection: TextDirection.ltr)..layout();
        textPainterX.paint(canvas, Offset(posX - (textPainterX.width / 2), centroY + 8));

        // MARCAS EN EJE Y (Vertical) - Positivo hacia arriba
        double posY = centroY - (i * escalaRegla);
        canvas.drawLine(Offset(centroX - 5, posY), Offset(centroX + 5, posY), paintRegla);
        
        final textSpanY = TextSpan(
          text: i.toString(),
          style: const TextStyle(color: Colors.white70, fontSize: 10),
        );
        final textPainterY = TextPainter(text: textSpanY, textDirection: TextDirection.ltr)..layout();
        // Colocamos el texto ligeramente a la izquierda del eje vertical
        textPainterY.paint(canvas, Offset(centroX - textPainterY.width - 8, posY - (textPainterY.height / 2)));
      }
    } else {
      // --- MODO 1D: Regla Lineal Horizontal ---
      canvas.drawLine(Offset(0, centroY), Offset(size.width, centroY), paintRegla);
      ...
      for (int i = -10; i <= 10; i++) {
        ...
        textPainter.paint(canvas, Offset(posX - (textPainter.width / 2), centroY + 10));
      }
    }
